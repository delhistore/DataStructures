...
void printLevel(Node * root) {      //O(N)
    ...
    queue<Node *> q;
    q.push(root);
    q.push(NULL);                                   // NULL is pushed to mark the end of current level

    while (q.empty() == false) {                    // Queue will be empty when the last NULL for the last level has been popped
        Node * cur = q.front();
        q.pop();

        if (cur == NULL) {                          // whenever current element is NULL we have reached the end of current level
            cout << endl;
            if (!q.empty()) q.push(NULL);           // if queue has elements from next level push NULL to end of queue to mark 
            continue;                               // end of current level
        }
